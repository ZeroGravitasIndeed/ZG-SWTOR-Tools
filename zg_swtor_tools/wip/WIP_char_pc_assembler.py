...
class ZGSWTOR_OT_pc_assembler(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if not zipfile.is_zipfile(f"{ADDON_ROOT}/resources/pcs_str.zip"):
            bpy.context.window.cursor_set("DEFAULT")
            self.report({"WARNING"}, "pcs_str.zip file not found.")
            return {"CANCELLED"}
        with zipfile.ZipFile(f"{ADDON_ROOT}/resources/pcs_str.zip") as pcs_str_zip:
        ...
        if not zipfile.is_zipfile(f"{ADDON_ROOT}/resources/gom_str.zip"):
            bpy.context.window.cursor_set("DEFAULT")
            self.report({"WARNING"}, "gom_str.zip file not found.")
            return {"CANCELLED"}
        with zipfile.ZipFile(f"{ADDON_ROOT}/resources/gom_str.zip") as gom_str_zip:
            gom_file = gom_str_zip.open("gom_str.json", "r")
        gom = json.load(gom_file)


        # # Types strings (for identifying the PCS nodes' data types).
        # if not zipfile.is_zipfile(f"{ADDON_ROOT}/resources/types.zip"):
        #     bpy.context.window.cursor_set("DEFAULT")
        #     self.report({"WARNING"}, "types.zip file not found.")
        #     return {"CANCELLED"}
        # types_zip = zipfile.ZipFile(f"{ADDON_ROOT}/resources/types.zip")
        # types_file = types_zip.open("types.json", "r")
        # types = json.load(types_file)


        # DYNAMIC strings (for identifying the PCS nodes' data types).
        if not zipfile.is_zipfile(f"{ADDON_ROOT}/resources/dynamic.zip"):
            bpy.context.window.cursor_set("DEFAULT")
            self.report({"WARNING"}, "dynamic.zip file not found.")
            return {"CANCELLED"}
        with zipfile.ZipFile(f"{ADDON_ROOT}/resources/dynamic.zip") as dynamic_zip:
        ...
        if not zipfile.is_zipfile(f"{ADDON_ROOT}/resources/pcs.zip"):
            bpy.context.window.cursor_set("DEFAULT")
            self.report({"WARNING"}, "pcs.zip file not found.")
            return {"CANCELLED"}
        with zipfile.ZipFile(f"{ADDON_ROOT}/resources/pcs.zip") as pcs_zip:
            pass
