...
def find_substrings(source, start, end):
    substrings = []
    start_index = source.find(start)
    while start_index != -1:
        end_index = source.find(end, start_index + len(start))
        if end_index != -1:
            substrings.append(source[start_index:end_index + len(end)])
            start_index = source.find(start, end_index + len(end))
        else:
            break
    return substrings
...
class ZGSWTOR_OT_outfit_assembler(bpy.types.Operator, ImportHelper):
    bl_label = "SWTOR Outfit Assembler"
    bl_idname = "zgswtor.outfit_assembler"
    bl_description = "Processes the URL of a Jedipedia.net's armor item page to access it,\nread its data about objects and materials, and import them"
    ...
    separate_eyes: bpy.props.BoolProperty(
        ...
        description="Separates the eyes from the character's head object to make them easier\nto pose and animate. Uses their material, if their own, as separating criteria.",
        default = True,
        # options={'HIDDEN'}
    )
    ...
    def execute(self, context):
        ...
        if self.gather_only == False:
            ...
            if self.separate_eyes:
                for obj in character_objects:
                    if "head" in obj.name:
                        eyes_and_head_objects = separate_eyes(obj)
                        break
                
                for obj in eyes_and_head_objects:
...
classes = [
    ZGSWTOR_OT_outfit_assembler,
]

def register():
    bpy.utils.register_class(ZGSWTOR_OT_outfit_assembler)
...
def unregister():
    bpy.utils.unregister_class(ZGSWTOR_OT_outfit_assembler) 
