...
REFERENCE_SWTOR_MAT_TEMPLATES_FILEPATH = Path(ADDON_ROOT) / 'resources' / 'default_materials_templates' / 'SWTOR Materials 4.2.blend'


# ---- SWTOR TEMPLATE MATERIALS' ANALYSIS -----------------------

# Simple lister of a SWTOR templates file's TOR materials.
...
def append_swtor_materials_from_template_blend_file(blend_filepath):
    # Get a list of all the TOR materials the game has
    # (reflected in the default SWTOR Slicers template)
    
    standard_set_of_swtor_materials_names = read_swtor_template_mats_names_in_file(REFERENCE_SWTOR_MAT_TEMPLATES_FILEPATH)

    # Delete all existing SWTOR materials templates
    for mat in list(bpy.data.materials):
        if mat.name.startswith("TOR-"):
            mat.use_fake_user = False
            bpy.data.materials.remove(mat.name)

    # Rename existing SWTOR node groups to secure them from collisions
    timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
    for ng in [ng for ng in bpy.data.node_groups if ng.name.startswith("TOR-") or ng.name.startswith("TOR_Aux-")]:
        if "-OLD" not in ng.name:
            ng.name = f"{ng.name}-OLD{timestamp}"
            ng.use_fake_user = False

    # Append new SWTOR materials templates (which also brings their own NGs)
    swtor_mats_names = []
...
def append_materials_from_template_blend_file_COMPLEX(blend_filepath, update_if_full_match=False, use_fake_user=True):
    """
    (Blender-generic)
    ...
    * Can add fake users to the loaded materials if wished so.
    ...
    if full_match_exists:
        if update_if_full_match:
            ...
            if not compare_internal_and_external_swtor_mat_templates(blend_filepath):
                ...
                for ng in ngs_to_timestamp:
                    ...
                    bpy.data.node_groups[ng].name = f"{bpy.data.node_groups[ng].name}.OLD{timestamp}"
...
def append_materials_from_template_blend_file_OLD(blend_filepath, update_if_full_match=False, use_fake_user=True):
    """
    (Blender-generic)
...
def append_materials_from_template_blend_file_OLD2(blend_filepath, use_fake_user=True):
    """
    (Blender-generic)
    ...
    * Can add fake users to the loaded materials if wished so.
    ...
    if f"TOR-{selected_swtor_template_mats_id}-Uber" in bpy.data.node_groups:
        protect_previous_swtor_node_groups = compare_internal_and_external_swtor_mat_templates(blend_filepath) is not None
